import { assertSpatialPermission, getSpatialObject, type Vec3, type WorldGraph } from './world-graph.js';
...
export function applySpatialMutation(graph: WorldGraph, mutation: SpatialMutation): WorldGraph {
  ...
  const objects = graph.objects.map((entry) => {
    ...
    switch (mutation.kind) {
      case 'translate': return { ...entry, position: mutation.position };
      case 'rotate': return { ...entry, rotation: mutation.rotation };
      case 'material': return { ...entry, material: mutation.material };
      case 'intensity': return { ...entry, intensity: Math.max(0, Math.min(100, mutation.intensity)) };
    }
  });
